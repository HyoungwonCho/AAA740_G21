...
class Agent:
    ...
    def setup_wandb(self):
        if WANDB_ENABLE and not self.args.debug:
            setup_wandb(self.args, project=self.args.wandb_project, name=self.args.project_name)
        ...
        if self.optimizer is not None:
            if isinstance(self.optimizer, list):
                for i in range(len(self.optimizer)):
                    self.optimizer[i].load_state_dict(complex_to_device(self.optimizer[i].state_dict(), device=self.device))
            else:
    ...
    def train(self, train_loader, eval_loader=None, use_tqdm=None, inner_collect_fn=None):
        if self.args.resume:
            if not self.args.deepspeed:
        ...
        for epoch in epoch_iter:
            self.epoch = epoch
            # Train phase
            train_meter, train_time = self.train_fn(train_loader, use_tqdm=use_tqdm)
            logger.info("[Rank %s] Train Epoch: %d/%d, Time: %s\n %s" % (self.rank, epoch + 1, self.args.epochs, train_time, train_meter.avg))
            ...
            if (epoch + 1) % self.args.eval_step == 0:
                if eval_loader:
                    ...
                    logger.info("[Rank %s] Valid Epoch: %d/%d, Time: %s\n %s" % (self.rank, epoch + 1, self.args.epochs, eval_time, eval_meter.avg))
    ...
    def eval_fn_visualization(
        ...
        with T.no_grad():
            ...
            for batch_idx, inputs in enumerate(eval_loader):
                ...
                images_list = [image.squeeze() for image in outputs["logits_imgs"]]
                images_name_list = ["results/" + "_".join(name.split("/")[-3:]) for name in inputs["img_key"]]

                labels_list = [image.squeeze() for image in inputs["label_imgs"]]
                labels_name_list = ["labels/" + "_".join(name.split("/")[-3:]) for name in inputs["img_key"]]

                reference_list = [image.squeeze() for image in inputs["reference_img"]]
                reference_name_list = ["reference/" + name.split("/")[-1] for name in inputs["img_key"]]

                cond_list = [image.squeeze() for image in inputs["cond_imgs"]]
                conditions_name_list = ["conditions/" + name.split("/")[-1] for name in inputs["img_key"]]

                # cond_list = [image.squeeze() for image in inputs["cond_edge_imgs"]]
                # conditions_name_list = ["conditions/edge_" + name.split("/")[-1] for name in inputs["img_key"]]

                # cond_list = [image.squeeze() for image in inputs["cond_depth_imgs"]]
                # conditions_name_list = ["conditions/depth_" + name.split("/")[-1] for name in inputs["img_key"]]

                # save results
                for image, name in zip(images_list, images_name_list):
                    iden = name.split("/")[0]
                    if not os.path.exists(eval_save_filename + "/" + iden):
                        os.makedirs(eval_save_filename + "/" + iden)
                    save_image(image, eval_save_filename + "/" + name)
                # save labels
                for image, name in zip(labels_list, labels_name_list):
                    iden = name.split("/")[0]
                    if not os.path.exists(eval_save_filename + "/" + iden):
                        os.makedirs(eval_save_filename + "/" + iden)
                    save_image(image, eval_save_filename + "/" + name)
                # save reference
                for image, name in zip(reference_list, reference_name_list):
                    iden = name.split("/")[0]
                    if not os.path.exists(eval_save_filename + "/" + iden):
                        os.makedirs(eval_save_filename + "/" + iden)
                    save_image(image, eval_save_filename + "/" + name)
                # save conditions
                for image, name in zip(cond_list, conditions_name_list):
                    iden = name.split("/")[0]
                    if not os.path.exists(eval_save_filename + "/" + iden):
                        os.makedirs(eval_save_filename + "/" + iden)
                    save_image(image, eval_save_filename + "/" + name)
                ...
                # if inner_collect_fn and self.enable_collect:
                #     remove_key = inputs.pop("enc_dec_only", None)
                #     gt_save_path, pred_save_path = inner_collect_fn(self.args, inputs, outputs, self.log_dir, self.global_step, eval_save_filename)
    ...
    def get_eval_metrics(self, eval_meter, eval_save_filename, gt_save_path=None, pred_save_path=None):
        ...
        if os.path.exists(gt_folder) and os.path.exists(pred_folder):
            ...
            logger.warning(f"Failed to calculate FID, gt {gt_folder}, {os.path.exists(gt_folder)}\npred {pred_folder}, {os.path.exists(pred_folder)}")
    ...
    def train_eval_by_iter(self, train_loader, eval_loader=None, use_tqdm=True, inner_collect_fn=None):
        ...
        while self.global_step < len(train_loader):
            ...
            if (self.global_step + 1) % self.args.iter_per_ep == 0:
                logger.info(
                    "[Rank %s] Train Epoch: %d/%d, Time: %s\n %s" % (self.rank, self.epoch + 1, self.args.epochs, train_time, train_meter.avg)
                )
                if not isinstance(train_meter.avg, dict):
                    raise ValueError(type(train_meter.avg))
                tr_meter_log = {
        ...
        if (self.epoch + 1) % self.args.eval_step != 0:
            ...
            if eval_loader:
                eval_meter, eval_time = self.eval_fn(eval_loader, inner_collect_fn=inner_collect_fn, use_tqdm=use_tqdm)
                logger.info("[Rank %s] Valid Epoch: %d/%d, Time: %s\n %s" % (self.rank, self.epoch + 1, self.args.epochs, eval_time, eval_meter.avg))
                # Update metric with eval metrics
                val_meter_log = {**eval_meter.avg}
                self.log_dict_to_wandb({f"{k}_ep": v for k, v in val_meter_log.items()})
                val_meter_log.update({"time": eval_time})
    ...
    def check_outputs(self, outputs):
        error_message = (
            'Model output must be a dict. The key must be "class_subclass" format.'
    ...
    def load_checkpoint(self, checkpoint_filename):
        if not self.args.deepspeed:
            ...
            logger.warning(
                "Loaded checkpoint %s of epoch %s (global_step %s)" % (checkpoint_filename, checkpoint["epoch"], checkpoint["global_step"])
            )
        else:
    ...
    def load_checkpoint_for_deepspeed_diff_gpu(self, checkpoint_filename):
        ...
        # IMPORTANT! The models will be wrapped automatically.
        logger.warning("Loaded checkpoint %s of global_step %s" % (checkpoint_filename, checkpoint["global_steps"]))
    ...
    def save_checkpoint(self, checkpoint_filename):
        if not self.args.deepspeed:
            ...
            logger.warning("Saved epoch %s (global_step %s) to %s." % (checkpoint["epoch"], checkpoint["global_step"], checkpoint_filename))
            return
        else:
            if self.args.debug and checkpoint_filename != self.last_checkpoint_filename:
                logger.warning(f"skip saving models with deepspeed to " f"{self.log_dir}/{checkpoint_filename} in debug mode")
                return
    ...
    def log_memory(self, ep=-1, step=-1):
        ...
        return f"{step_str} lr_base: {lr_base}, " + f"lr_head: {lr_head}, lr_xmodal: {lr_xmodal}, max memory: {memory}"
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    ...
                    if not any(nd in n for nd in no_decay) and not any(bb in n for bb in head_names) and not any(ht in n for ht in cross_modal_names)
                ],
                "weight_decay": wd,
                "lr": lr,
            },
            {
                "params": [
                    p
                    for n, p in param_optimizer
                    if any(nd in n for nd in no_decay) and not any(bb in n for bb in head_names) and not any(ht in n for ht in cross_modal_names)
                ],
                "weight_decay": 0.0,
                "lr": lr,
            },
            {
                "params": [
                    p
                    for n, p in param_optimizer
                    if not any(nd in n for nd in no_decay) and any(bb in n for bb in head_names) and not any(ht in n for ht in cross_modal_names)
                ],
                "weight_decay": wd,
                "lr": lr * lr_mult_head,
            },
            {
                "params": [
                    p
                    for n, p in param_optimizer
                    if any(nd in n for nd in no_decay) and any(bb in n for bb in head_names) and not any(ht in n for ht in cross_modal_names)
                ],
                "weight_decay": 0.0,
                "lr": lr * lr_mult_head,
            },
            {
                "params": [
                    p
                    for n, p in param_optimizer
                    if not any(nd in n for nd in no_decay) and not any(bb in n for bb in head_names) and any(ht in n for ht in cross_modal_names)
                ],
                "weight_decay": wd,
            ...
            },
            {
                "params": [
                    p
                    for n, p in param_optimizer
                    if any(nd in n for nd in no_decay) and not any(bb in n for bb in head_names) and any(ht in n for ht in cross_modal_names)
                ],
                "weight_decay": 0.0,
                "lr": lr * lr_mult_cross_modal,
            },
        ]
        optzr = T.optim.AdamW(optimizer_grouped_parameters, lr=lr, betas=(0.9, 0.98))
        return optzr
...
class Agent_LDM(Agent):
    ...
    def train(self, train_loader, eval_loader=None, use_tqdm=True, inner_collect_fn=None):
        if self.args.resume:
            if not self.args.deepspeed:
                if os.path.exists(os.path.join(self.log_dir, self.last_checkpoint_filename)):
                    logger.warning(f"Resuming.... from {self.last_checkpoint_filename}")
