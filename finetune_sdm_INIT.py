...
from agent_INIT import Agent_LDM, WarmupLinearLR, WarmupLinearConstantLR
import os
import torch
...
def main_worker(args):
    ...
    if args.do_train:
        ...
        if getattr(args, "refer_clip_preprocess", None):
            train_dataset = BaseDataset(args, split="train", preprocesser=model.feature_extractor)
            eval_dataset = BaseDataset(args, split="val", preprocesser=model.feature_extractor)
        else:
            train_dataset = BaseDataset(
                args, split="train"
            )  # video_root = "/media/data/HDTF_preprocessed/30_frame/HDTF/" kpt=잘해뒀음.. 잘.. @ config/ref_attn_clip_combine_controlnet/HDTF_kpt.py
            eval_dataset = BaseDataset(args, split="val")

        train_info = get_loader_info(args, args.local_train_batch_size, train_dataset)

        _, images_per_batch, args.iter_per_ep, args.num_iters = train_info  # (32, 32, 2837, 56740)
        ...
        train_dataloader, train_info = make_data_loader(
            args,
            args.local_train_batch_size,
            train_dataset,
            start_iter=trainer.global_step + 1,
            loader_info=train_info,
        )
