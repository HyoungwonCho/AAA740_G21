...
class BaseDataset(Dataset):
    def __init__(self, args, split, debugging=False, preprocesser=None):
        ...
        total_dataset_roots = [img for img in glob(dataset_root, recursive=True) if os.path.isfile(img)]
        self.from_weather = self.eval_class.split('2')[0]
        self.to_weather = self.eval_class.split('2')[-1]
        ...
        self.ref_dataset_roots = [root for root in total_dataset_roots if self.to_weather in root]
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            # Depth
            depth_img_name = img_name.replace("/image/", "/depth/")  # /media/dataset/init_hw/depth/sunny/sunny_0000.png
