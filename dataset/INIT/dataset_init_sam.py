...
class BaseDataset(Dataset):
    def __init__(self, args, split, debugging=False, preprocesser=None):
        ...
        if debugging:
            dataset_root = "/media/data/init_hw/image/*/*.png"
        ...
        dataset_roots = [img for img in glob(dataset_root) if os.path.isfile(img)]
        dataset_roots.sort()
        random.seed(90)
        random.shuffle(dataset_roots)

        if split == "train":
            all_images = dataset_roots[:-50]
        else:
            all_images = dataset_roots[-50:]
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            class_name = img_name.split("/")[-2]  # rainy
            ...
            reference_img_controlnet = self.augmentation(input_rgb, self.transform, state)  # not used

            # ControlNet condition
            condition_img_name = img_name.replace("image", "mask")
            condition_img_name = condition_img_name.replace(
                class_name, class_name + "_mask"
            )  # /media/data/init_hw/mask/rainy_mask/rainy_mask_0123.png
            input_segmentation = Image.open(condition_img_name).convert("RGB")
            condition_img = self.augmentation(input_segmentation, self.cond_transform, state)  # condition_img: ControlNet input

            # add mask
            # /media/data/init_hw/image/rainy/rainy_0123.png
            mask_only_img_name = img_name.replace("image/", "mask_only/")
            mask_only_img_name = mask_only_img_name.replace(f"{class_name}", f"{class_name}_mask_only").replace(
                "only_", ""
            )  # /media/data/init_hw/mask_only/rainy_mask_only/rainy_mask_0123.png
            input_segmentation = Image.open(mask_only_img_name).convert("RGB")
            bin_mask = self.process_mask(input_segmentation)
            img_array = np.array(input_rgb)

            foreground = Image.fromarray(img_array * bin_mask)
            bin_mask = self.augmentation(bin_mask, self.mask_transform, state)
