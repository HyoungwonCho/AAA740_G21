...
class BaseDataset(Dataset):
    def __init__(self, args, split, debugging=False, preprocesser=None):
        ...
        dataset_roots = [
            img for img in glob(dataset_root, recursive=True) if os.path.isfile(img) and os.path.isfile(img.replace("/init/", "/init_depth/"))
        ]
        ...
        print("The length of the dataset roots is: ", len(dataset_roots))
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            class_name = img_name.split("/")[4]  # sunny
            input_rgb = Image.open(img_name).convert("RGB")
            label_img = self.augmentation(input_rgb, self.transform, state)  # label_img: GT

            # Seg
            # seg_image_name = img_name.replace("/image/", "/mask_only/") # /media/dataset/init_hw/mask_only/sunny/sunny_0000.png
