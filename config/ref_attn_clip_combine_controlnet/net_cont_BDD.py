...
import wandb
from utils.wutils_ldm import adaptively_load_state_dict, file2data
# import sys
# sys.path.append("...")
...
class Net(nn.Module):
    def __init__(self, args):
        ...
        controlnet = ControlNetModel.from_unet(unet=unet, args=self.args)
...
def inner_collect_fn(args, inputs, outputs, log_dir, global_step, eval_save_filename="eval_visu"):
    ...
    for _, sample in enumerate(ld):
        ...
        if "cond_imgs" in sample:  # pose
            ...
            try:
                image.save(os.path.join(cond_save_path, prefix + ".png"))
            except Exception as e:
                print(f"some errors happened in saving logits_imgs: {e}")
