...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.ddpm_feature_extractor = FeatureExtractorDDPM(model=unet, blocks=[5, 6, 7, 8, 12], input_activations=False, **unet.config)
    ...
    def forward_train_multicontrol(self, inputs, outputs):
        ...
        x_t_features = self.ddpm_feature_extractor.get_activations(noisy_latents, timesteps, refer_latents)
        x_0_features = self.ddpm_feature_extractor.get_activations(latents, timesteps, refer_latents)
        ...
        outputs["loss_total"] = mse_loss + loss_zecon * 0.5
...
def inner_collect_fn(args, inputs, outputs, log_dir, global_step, eval_save_filename="eval_visu"):
    ...
    for _, sample in enumerate(ld):
        ...
        if "cond_edge_imgs" in sample:  # pose
            image = tensor2pil(sample["cond_edge_imgs"])[0]
            postfix = "_edge"
            try:
        ...
        if "cond_depth_imgs" in sample:  # pose
            image = tensor2pil(sample["cond_depth_imgs"])[0]
            postfix = "_depth"
            try:
                image.save(os.path.join(cond_save_path, prefix + postfix + ".png"))
            except Exception as e:
