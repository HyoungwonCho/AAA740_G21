...
class Net(nn.Module):
    def __init__(self, args):
        ...
        if self.args.pretrained_model_dreambooth:
            print(f"Loading pre-trained unet from {self.args.pretrained_model_dreambooth}/unet")
            unet = UNet2DConditionModel.from_pretrained(self.args.pretrained_model_path, subfolder="unet")
            pretrained_dict = file2data(self.args.pretrained_model_dreambooth, map_location="cpu")
            adaptively_load_state_dict(unet, pretrained_dict["module"])
        else:
        ...
        controlnet_unit = MultiControlNetModel_MultiHiddenStates([controlnet_edge, controlnet_depth])
    ...
    def forward_train_multicontrol(self, inputs, outputs):
        ...
        controlnet_edge_image = inputs["cond_edge_imgs"].to(dtype=self.dtype)
        controlnet_depth_image = inputs["cond_depth_imgs"].to(dtype=self.dtype)
        controlnet_image = [controlnet_edge_image, controlnet_depth_image]
    ...
    def forward_sample_multicontrol(self, inputs, outputs):
        ...
        cond_image = [image_edge, image_depth]
