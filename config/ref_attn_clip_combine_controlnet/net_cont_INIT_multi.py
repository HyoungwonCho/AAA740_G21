...
class Net(nn.Module):
    def __init__(self, args):
        ...
        controlnet_seg = ControlNetModel.from_unet(unet=unet, args=self.args)
        controlnet_edge = ControlNetModel.from_unet(unet=unet, args=self.args)
        controlnet_depth = ControlNetModel.from_unet(unet=unet, args=self.args)
        ...
        # if args.ref_null_caption:  # False: 기본 세팅
        ...
        #     controlnet_background = ControlNetModel.from_unet(
        ...
        controlnet_unit = MultiControlNetModel_MultiHiddenStates([controlnet_seg, controlnet_edge, controlnet_depth])
        ...
        if getattr(self.args, "combine_clip_local", None) and not getattr(
            ...
            self.refer_clip_proj = torch.nn.Linear(
                clip_image_encoder.visual_projection.in_features, clip_image_encoder.visual_projection.out_features, bias=False
            )
    ...
    def text_encode(self, prompt, num_images_per_prompt=1, do_classifier_free_guidance=False, negative_prompt=None):
    ...
    #     controlnet_seg_images = inputs["cond_seg_imgs"].to(dtype=self.dtype)
    #     controlnet_edge_image = inputs["cond_edge_imgs"].to(dtype=self.dtype)
    #     controlnet_depth_image = inputs["cond_depth_imgs"].to(dtype=self.dtype)

    #     down_block_res_samples, mid_block_res_sample = self.controlnet(
    #         noisy_latents, timesteps, z_text, controlnet_cond=controlnet_image, return_dict=False
    #     )
    ...
    def forward_train_multicontrol(self, inputs, outputs):
        ...
        controlnet_edge_image = inputs["cond_edge_imgs"].to(dtype=self.dtype)
        controlnet_depth_image = inputs["cond_depth_imgs"].to(dtype=self.dtype)
        controlnet_image = [controlnet_seg_images, controlnet_edge_image, controlnet_depth_image]
        ...
        if self.args.ref_null_caption:
            ...
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                noisy_latents,
                timesteps,
                [refer_latents, refer_latents],  # both controlnet path use the refer latents
                controlnet_cond=controlnet_image,
                conditioning_scale=[1.0, 1.0],
                return_dict=False,
            )
    ...
    #     #     do_classifier_free_guidance=do_classifier_free_guidance,
    #     # )
    #     image_seg = self.prepare_image(
    #         image=inputs["cond_seg_imgs"].to(dtype=self.dtype),
    #         width=w,
    #         height=h,
    #         batch_size=b * self.args.num_inf_images_per_prompt,
    #         num_images_per_prompt=self.args.num_inf_images_per_prompt,
    ...
    #         do_classifier_free_guidance=do_classifier_free_guidance,
    #     )
    #     image_edge = self.prepare_image(
    #         image=inputs["cond_edge_imgs"].to(dtype=self.dtype),
    #         width=w,
    #         height=h,
    #         batch_size=b * self.args.num_inf_images_per_prompt,
    #         num_images_per_prompt=self.args.num_inf_images_per_prompt,
    ...
    #         do_classifier_free_guidance=do_classifier_free_guidance,
    #     )
    #     image_depth = self.prepare_image(
    #         image=inputs["cond_depth_imgs"].to(dtype=self.dtype),
    #         width=w,
    #         height=h,
    #         batch_size=b * self.args.num_inf_images_per_prompt,
    #         num_images_per_prompt=self.args.num_inf_images_per_prompt,
    ...
    def forward_sample_multicontrol(self, inputs, outputs):
        ...
        cond_image = [image_seg, image_edge, image_depth]
...
def inner_collect_fn(args, inputs, outputs, log_dir, global_step, eval_save_filename="eval_visu"):
    ...
    for _, sample in enumerate(ld):
        ...
        if "cond_seg_imgs" in sample:  # pose
            image = tensor2pil(sample["cond_seg_imgs"], resize_img=args.pos_resize_img, img_target_size=ref_img_size)[0]
            postfix = '_seg'
            try:
                image.save(os.path.join(cond_save_path, prefix + postfix + ".png"))
            except Exception as e:
                print(f"some errors happened in saving logits_imgs: {e}")
        if "cond_edge_imgs" in sample:  # pose
            image = tensor2pil(sample["cond_edge_imgs"], resize_img=args.pos_resize_img, img_target_size=ref_img_size)[0]
            postfix = '_edge'
            try:
                image.save(os.path.join(cond_save_path, prefix + postfix + ".png"))
            except Exception as e:
