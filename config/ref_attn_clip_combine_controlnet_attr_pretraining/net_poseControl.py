    ...
    def forward_train_multicontrol(self, inputs, outputs):
        ...
        if self.args.ref_null_caption:
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                noisy_latents, timesteps, z_text, controlnet_cond=controlnet_image, return_dict=False  # reference controlnet path use null caption
            )
        else:
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                noisy_latents,
        ...
        recon = torch.from_numpy(recon).float().to(self.device)

        self.netD = self.netD.cuda().to(self.device)
        self.criterionGAN = self.criterionGAN.cuda().to(self.device)
        # import pdb;pdb.set_trace()
        d_loss = compute_D_loss(
            recon.float(), ref_image.float().cuda().to(self.device), self.netD.float().to(self.device), self.criterionGAN.float().to(self.device)
        )
...
def inner_collect_fn(args, inputs, outputs, log_dir, global_step, eval_save_filename="eval_visu"):
    ...
    pred_save_path = os.path.join(
        eval_log_dir, f"pred_gs{args.guidance_scale}_scale-cond{args.controlnet_conditioning_scale_cond}-ref{args.controlnet_conditioning_scale_ref}"
    )
