...
class Net(nn.Module):
    def __init__(
        ...
        #netD = PatchDiscriminator(input_nc=3, ndf=64, n_layers=3, norm_layer=functools.partial(nn.InstanceNorm2d, affine=False, track_running_stats=False), no_antialias=False)
        # EDIT END
    ...
    def forward_train_multicontrol(self, inputs, outputs):
        ...
        original_latents = self.noise_scheduler.step(model_pred.to('cpu'), timesteps.to('cpu').detach().numpy().all(), latents.to('cpu')).pred_original_sample
        ...
        #d_loss = compute_D_loss(recon.float(), ref_image.float().cuda().to(self.device), self.netD.float().to(self.device), self.criterionGAN.float().to(self.device))
        
        loss_feature = self.feat_loss(self.img_norm(image.half().cuda().to(self.device)), self.img_norm(recon.half().cuda().to(self.device)))
        ...
        outputs['loss_total'] = mse_loss + loss_feature * 0.5
        #outputs['loss_vgg'] = loss_feature * 0.5
        # outputs['loss_cut'] = loss_zecon 
