...
from .unet_2d_condition_adain import UNet2DConditionModel
import PIL.Image
from .controlnet_adain import ControlNetModel
...
class Net(nn.Module):
    ...
    def clip_encode_image_global(
        self, image, num_images_per_prompt=1, do_classifier_free_guidance=False
    ):  # clip global feature
    ...
    def clip_encode_image_local(
        ...
        last_hidden_states_norm = self.clip_image_encoder.vision_model.post_layernorm(
            last_hidden_states
        )
    ...
    def prepare_latents(
        self, batch_size, num_channels_latents, height, width, generator, latents=None
    ):
        shape = (
            batch_size,
    ...
    def zecon_loss(self, x0_features_list, x0_t_features_list, temperature=0.07):
        ...
        for x0_features, x0_t_features in zip(x0_features_list, x0_t_features_list):
            ...
            layer_loss = F.cross_entropy(
                logits_1d.view(batch_size, -1), labels_1d, reduction="mean"
            )
    ...
    def forward_train_multicontrol(self, inputs, outputs):
        ...
        '''AdaIN 주입'''
        style = self.ref_image_encoder(ref_image)
        style = style.to(dtype=self.dtype)

        timesteps = torch.randint(
            0, self.tr_noise_scheduler.num_train_timesteps, (bsz,), device=latents.device
        )
        ...
        if self.args.ref_null_caption:
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                noisy_latents,
                timesteps,
                z_text,  # reference controlnet path use null caption
            ...
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                noisy_latents,
                timesteps,
                style,  # both controlnet path use the refer latents
                controlnet_cond=controlnet_image,
                return_dict=False,
            )
        ...
        outputs["loss_total"] = mse_loss * 2.0 + loss_feature * 1.0 + g_loss * 0.1
    ...
    def forward_sample_multicontrol(self, inputs, outputs):
        ...
        if self.args.combine_clip_local:
            refer_latents = self.clip_encode_image_local(
                ref_image, self.args.num_inf_images_per_prompt, do_classifier_free_guidance
            )
        else:
        ...
        if self.args.use_adain:
            style = self.style_encoder(ref_image).to(dtype=self.dtype)
            # print(style.shape)
            # duplicate image embeddings for each generation per prompt, using mps friendly method
            bs_embed, seq_len, _, _ = style.shape
            style = style.repeat(1, self.args.num_inf_images_per_prompt, 1, 1)
            style = style.view(bs_embed * self.args.num_inf_images_per_prompt, seq_len)
        else:
            style = None
...
def compute_D_loss(fake_B, real_B, netD, criterionGAN):
    ...
    pred_real = netD(
        real_B
    )  # , self.netG.ContentEncoder.reflectionpad_1, self.netG.ContentEncoder.deform_conv_1, pano=False, pin=True)
