...
from .transformer_2d_adain import Transformer2DModel as AdaINformer
...
def get_down_block(
    ...
    elif down_block_type == "AdaINDownBlock2D":
        return AdaINDownBlock2D(
            num_layers=num_layers,
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            out_channels=out_channels,
...
# [aiku] 
class AdaINDownBlock2D(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(
        ...
        hidden_states: torch.FloatTensor,
        temb: Optional[torch.FloatTensor] = None,
        style: Optional[torch.FloatTensor] = None,
        attention_mask: Optional[torch.FloatTensor] = None,
        encoder_attention_mask: Optional[torch.FloatTensor] = None,
        additional_residuals=None,
    ):
...
# [aiku]
class AdaINCrossAttnDownBlock2d(nn.Module):
    ...
        ...
        if self.upsamplers is not None:
            for upsampler in self.upsamplers:
                if self.upsample_type == "resnet":
                    hidden_states = upsampler(hidden_states, temb=temb, upsample_size=upsample_size)
                else:
